...
export async function cadastrarProduto(
  ...
  const [resposta] = await conexao.query(
    "INSERT INTO produtos (nome, descricao, quantidade, unidade, criado_por) VALUES (?, ?, ?, ?, ?)",
...
export async function atualizarProduto(
  ...
  const conexao = await pool.getConnection();
  const resposta = await conexao.query(
    "UPDATE produtos SET nome = ?, descricao = ?, quantidade = ?, unidade = ?, atualizado_por = ?, status = ? WHERE id = ?",
